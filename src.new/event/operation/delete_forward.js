...
export default function deleteForward(){
	let { rangeApi, nodeApi, operationEvent } = this,
	...
	if( node.nodeType === Node.TEXT_NODE ){
		...
		if( offset > 1 || (offset === 1 && node.length > 1) ){//	正常删除一个字符
			...
			if( nodeApi.isStartInContainer(singleNode) ){
				...
				nodeApi.getContainer(singleNode).dispatchEvent( operationEvent.deleteForwardOnStart );
			}else{
		...
		if( offset > 1 || (offset === 1 && node.childNodes.length > 1) ){//	正常删除一个元素
			...
			if( nodeApi.isContainer(node) ){
				...
				node.dispatchEvent( operationEvent.deleteForwardOnStart );
			}else{
				if( nodeApi.isStartInContainer(singleNode) ){
					console.log('根据 container 所在的组件, 触发对应的 deleteForwardOnStart 事件');
					nodeApi.getContainer(node).dispatchEvent( operationEvent.deleteForwardOnStart );
				}else{
